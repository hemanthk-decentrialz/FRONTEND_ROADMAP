  ...
  return (
    <header className="glass sticky top-0 z-30 flex items-center justify-between border-b px-5 py-4 backdrop-blur-xl" style={{ borderColor: "var(--border)", }}>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold" style={{ color: "var(--foreground)", }} suppressHydrationWarning>
            {greeting}
          </h2>
      ...
      <div className="flex items-center gap-4">
        <button className="surface surface-hover flex items-center gap-3 rounded-2xl px-3 py-2">
          ...
          <div className="hidden text-left md:block">
            <p className="text-sm font-semibold" style={{ color: "var(--foreground)", }} suppressHydrationWarning>
              {user?.name}
            </p>
          ...
          <FontAwesomeIcon icon={faChevronDown} className="text-muted hidden text-xs md:block"/>
        </button>
        <button
          onClick={() => {
            logout();
